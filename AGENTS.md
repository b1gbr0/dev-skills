...
Skills here are many and narrow — one topic each, with a `description` that names what the
skill does NOT cover and where to go instead. `samber/cc-skills-golang` is the prior art: dozens
of small skills rather than one big one per language. It works because a skill that does not
match its trigger costs only its `description`, while a broad skill fires on everything and
buries the one rule the agent actually needed.

So a language set is built in slices, never in one go. The first slice is the base — what
changes the agent's output in every single file (style, ownership, the standard library, errors,
build, tests). Cross-cutting topics that only some code needs — concurrency, performance, API
design, templates — are later slices, one plan each. A slice is exactly what one plan closes;
everything past it waits in `docs/plans/backlog/`.
